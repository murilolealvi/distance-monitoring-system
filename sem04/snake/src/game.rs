...
impl Game{
    pub fn new(stdout: Stdout, width: u16, height: u16) -> Self {
        let original_terminal_size: (u16, u16) = size().unwrap(); //unwrap takes value from Some
        Self{
            stdout,
    ...
    fn prepare_ui(&mut self) {
        enable_raw_mode().unwrap(); //raw mode to pass input directly to app without terminal driver
        self.stdout
    ...
    fn wait_for_key_event(&self, wait_for: Duration) -> Option<KeyEvent> {
        if poll(wait_for).ok()? { 
            //? is a operator for error handling
            // let result = get_an_option(); 
            // let val = match result {         --->   let val = get_an_option()?;
            //     Some(value) => value,
            //     None => return None,
            // }
